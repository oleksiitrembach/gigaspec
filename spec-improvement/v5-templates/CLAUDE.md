...
```
Step 1: Read STATE.md completely
Step 2: Read AGENT.md completely  
Step 3: Verify understanding with: "I have read STATE.md and AGENT.md. Current phase: [X]. Next task: [Y]."
Step 4: Wait for user confirmation before proceeding
```
...
|--------|---------------------|------------------|
| Code changes | `npm test` or equivalent | All tests pass |
| File edits | `npm run lint` | No lint errors |
| Type changes | `npx tsc --noEmit` | No type errors |
| New feature | Test coverage check | ≥ {{COVERAGE_TARGET}}% |
| STATE update | Check STATE.md format | Valid markdown |
...
1. ❌ **Deleting files without backup** - Always create `.backup/[filename].[timestamp]` first
...
1. **Use tools for verification** - Run tests, lint via MCP
2. **Report tool results** - Show success/failure
3. **Fall back gracefully** - If MCP unavailable, use bash
